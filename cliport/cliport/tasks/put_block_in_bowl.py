...
class PutBlockInBowlTestColors(Task):
    def __init__(self):
        super().__init__()
        self.max_steps = 10
        self.pos_eps = 0.05
        self.lang_template = "put the {pick} block in a {place} bowl"
        self.task_completed_desc = "done placing a block in a bowl."
    
    def reset(self, env):
        super().reset(env)
        n_bowls = 1 # we are not testing sequential moves
        n_blocks = 1

        all_color_names = ['blue', 'red', 'green', 'yellow', 'brown', 'gray', 'cyan', 'orange', 'purple', 'pink', 'white']

        
        #print(all_color_names)
        #np_seed = np.random.get_state()[1][0] # get the value of seed
        #pick_color = 
        #place_color = 
        # np.array([pick_color, place_color])

        np.random.shuffle(all_color_names) # Shuffle the array in-place
        pick_color = all_color_names[0]
        np.random.shuffle(all_color_names) 
        place_color = all_color_names[0]
        selected_color_names = [pick_color, place_color]
        colors = [utils.COLORS[cn] for cn in selected_color_names]
        ...
        # Add distractors.
        n_distractors = 0
        max_distractors = 3 # add 2 blocks and bowls each as distractors
        while n_distractors < max_distractors:
            # add block
            #colors = distractor_block_colors 
            pose = self.get_random_pose(env, block_size)
            if not pose:
                continue
            obj_id = env.add_object(block_urdf, pose)
            np.random.shuffle(distractor_block_colors) # Shuffle the array in-place
            #color_block = random.sample(distractor_bowl_colors, 1) # colors[n_distractors % len(colors)]
            if not obj_id:
                continue
            p.changeVisualShape(obj_id, -1, rgbaColor=distractor_block_colors[0] + [1])

            
            # add bowl
            #colors = distractor_bowl_colors  
            pose = self.get_random_pose(env, bowl_size)
            if not pose:
                continue
            obj_id = env.add_object(bowl_urdf, pose)
            np.random.shuffle(distractor_bowl_colors) # Shuffle the array in-place
            #color_bowl = random.sample(distractor_bowl_colors, 1) # colors[n_distractors % len(colors)]
            if not obj_id:
                continue
            p.changeVisualShape(obj_id, -1, rgbaColor=distractor_bowl_colors[0] + [1])

        ...
        for i, bbox in enumerate(bboxes):
            ...
            if pose[0] is not None:
                ...
                try:
                    ...
                    # add object dimention for automated masking purpose, for SD
                    object_points[box_id], mesh_dim = self.get_mesh_object_points_yusuke(box_id)
                    #print(f'object points {mesh_dim}')
                    (x_min, x_max, y_min, y_max) = mesh_dim[0][0], mesh_dim[1][0], mesh_dim[0][1], mesh_dim[1][1]

                    object_descs.append(object_name)
                    self.object_dims.append((x_min, x_max, y_min, y_max))
...
class PackingSeenGoogleObjects(PackingSeenGoogleObjectsSeq):
    """Packing seen Google Object Group task. None sequential task"""
    ...
    def set_goals(self, object_descs, object_ids, object_points, repeat_category, zone_pose, zone_size):
        ...
        self.goals.append(([(object_id, (0, None))], np.int32([[1]]), [zone_pose],
                            ...
                            1))
        self.lang_goals.append(self.lang_template.format(obj=object_descs[obj_idx]))
        # note down object name for stable diffusion later
        self.object_name = object_descs[obj_idx]
        self.object_dim = self.object_dims[obj_idx]
        #print(f'self.object_dim {self.object_dim}')

        # Only one mistake allowed.
        self.max_steps = 2
    

class PackingUnseenGoogleObjects(PackingSeenGoogleObjectsSeq):
    """Packing Unseen Google Object task."""
...
class PackingUnseenGoogleObjectsUnseenContainer(PackingUnseenGoogleObjects):
    """Packing Unseen Google Object task."""
    ...
    def reset(self, env):
        ...
        # change the color of the container for the new task
        ...
        print(container_id)
        print(colors)
        p.changeVisualShape(container_id, -1, rgbaColor=colors[0] + [1])

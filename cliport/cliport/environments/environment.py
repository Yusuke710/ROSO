...
class Environment(gym.Env):
    ...
    def reset(self, color_change=False):
        ...
        # this change the backgound color
        if color_change:
            # choose to set the random background color 
            all_color_names = ['blue', 'red', 'green', 'yellow', 'brown', 'gray', 'cyan', 'orange', 'purple', 'pink', 'white']
            np.random.shuffle(all_color_names) 
            selected_color_names = [all_color_names[0]]
            colors = [utils.COLORS[cn] for cn in selected_color_names]

            # Load the content of the URDF file
            with open(os.path.join(self.assets_root, UR5_WORKSPACE_URDF_PATH), 'r') as urdf_file:
                urdf_content = urdf_file.read()

            # Update the RGBA values in the URDF content
            updated_urdf_content = urdf_content.replace(
                '<color rgba="0.2 0.2 0.2 1"/>',  # original DarkGrey color
                f'<color rgba="{colors[0][0]} {colors[0][1]} {colors[0][2]} 1"/>'
            )

            # Save the updated URDF content to a temporary file
            MODIFIED_UR5_WORKSPACE_URDF_PATH = os.path.join(self.assets_root, 'ur5', 'container_temp.urdf')
            with open(MODIFIED_UR5_WORKSPACE_URDF_PATH, 'w') as temp_urdf_file:
                temp_urdf_file.write(updated_urdf_content)

            pybullet_utils.load_urdf(
                p, MODIFIED_UR5_WORKSPACE_URDF_PATH, [0.5, 0, 0])
    
        # or keep it as black background
        else:
            pybullet_utils.load_urdf(
